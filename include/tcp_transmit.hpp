...
struct tcp_hook_t {
    ...
    {
        full_connect_id_t full_connect_id(in_packet._remote_info.value(), in_packet._local_info.value());
        std::optional<std::shared_ptr<full_tcb_t>> full_tcb = tcp_manager.query_and_register_full_tcb(full_connect_id);
        if (full_tcb) {
    ...
    void tcp_handle_close_state(l4_packet& in_packet, std::shared_ptr<full_tcb_t> in_tcb)
    {
        tcp_header_t in_tcp;
    ...
    void tcp_handle_listen_state(l4_packet& in_packet, std::shared_ptr<full_tcb_t> in_tcb)
    {
    ...
    void tcp_handle_syn_sent(l4_packet& in_packet, std::shared_ptr<full_tcb_t> in_tcb)
    {
    ...
    bool tcp_check_segment(l4_packet& in_packet, std::shared_ptr<full_tcb_t> in_tcb)
    {
        ...
        tcp_header_t in_tcp;
        in_tcp.consume(in_packet._payload->get_pointer());
        uint16_t segment_length = in_packet._payload->get_remaining_len() - in_tcp.header_length * 2;
        ...
        case TCP_TIME_WAIT:
            ...
            if (segment_length == 0 && in_tcb->receiver.window > 0) {
                ...
                if (in_tcb->receiver.next <= in_tcp.seq_no && in_tcp.seq_no < in_tcb->receiver.next + in_tcb->receiver.window) {
                    return true;
                } else {
                    return false;
                }
            ...
            if (segment_length > 0 && in_tcb->receiver.window > 0) {
                //* RCV.NXT =< SEG.SEQ < RCV.NXT+RCV.WND
                //*        or RCV.NXT =< SEG.SEQ+SEG.LEN-1 < RCV.NXT+RCV.WND
                if ((in_tcb->receiver.next <= in_tcp.seq_no && in_tcb->receiver.next + in_tcb->receiver.window)
                    || (in_tcb->receiver.next <= in_tcp.seq_no + segment_length - 1
                           && in_tcp.seq_no + segment_length - 1 < in_tcb->receiver.next + in_tcb->receiver.window)) {
                    return true;
                } else {
                    return false;
                }
