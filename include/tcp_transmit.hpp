...
struct tcp_hook_t {
  ...
  void tcp_handle_close_state(l4_packet& in_packet,
  ...
  void tcp_handle_listen_state(l4_packet& in_packet,
                               std::shared_ptr<full_tcb_t> in_tcb) {
  ...
  void tcp_handle_syn_sent(l4_packet& in_packet,
                           std::shared_ptr<full_tcb_t> in_tcb) {
  ...
  void tcp_state_transition(l4_packet& in_packet,
    ...
    if (in_tcp.SYN) {
      switch (in_tcb->state) {
           ...
           * responses, all segment queues should be flushed, the user should
           * also receive an unsolicited general "connection reset" signal,
    ...
    switch (in_tcb->state) {
         ...
         * segment text to user RECEIVE buffers.  Text from segments can be
         * moved into buffers until either the buffer is full or the segment is
