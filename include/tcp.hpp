...
struct tcp_hook_t {
  uint8_t fake_header_buf[tcp_fake_header_t::size()];
  void tcp_filter(std::optional<l4_packet>& in_packet) {
    uint32_t sum = 0;
    sum += util::ntoh(in_packet->_local_info->ip_addr.get_raw_ipv4());
    sum += util::ntoh(in_packet->_remote_info->ip_addr.get_raw_ipv4());
    sum += util::ntoh(in_packet->_proto);
    sum += util::ntoh((uint16_t)in_packet->_payload->get_remaining_len());

    tcp_header_t tcp_header;
    tcp_header.consume(in_packet->_payload->get_pointer());

    uint16_t checksum =
        util::checksum(in_packet->_payload->get_pointer(),
                       in_packet->_payload->get_remaining_len(), sum);

    tcp_header.checksum = checksum;
    tcp_header.produce(in_packet->_payload->get_pointer());
  }
};
...
class tcp : public protocol_interface<l4_packet> {
 public:
  ...
  circle_buffer<std::shared_ptr<full_tcb_t>> active_tcbs;
  static tcp& instance() {
    static tcp instance;
    return instance;
  }
  virtual std::optional<l4_packet> gather_packet() {
    std::optional<std::shared_ptr<full_tcb_t>> active_tcb =
        active_tcbs.pop_front();
    if (!active_tcb) return std::nullopt;
    return std::move(active_tcb.value()->gather_packet());
  }

  virtual int proto() { return PROTO; }

  virtual std::string tag() { return "TCP"; }
  ...
  void tcp_send_rst() {}
  void tcp_send_ctl(l4_packet& in_packet, std::shared_ptr<full_tcb_t> in_tcb,
                    int ctl, uint32_t seq_no, uint32_t ack_no) {
    tcp_header_t tcp_header;
    ...
    std::unique_ptr<packet> out_data =
        std::make_unique<packet>(tcp_header_t::size());
    tcp_header.produce(out_data->get_pointer());

    l4_packet out_packet(in_packet._remote_info, in_packet._local_info,
                         TCP_PROTO, std::move(out_data));
  ...
  bool tcp_handle_close_state(l4_packet& in_packet,
  ...
  bool tcp_handle_listen_state(l4_packet& in_packet,
                               std::shared_ptr<full_tcb_t> in_tcb) {
  ...
  bool tcp_handle_syn_sent(l4_packet& in_packet,
                           std::shared_ptr<full_tcb_t> in_tcb) {
  ...
  bool tcp_check_segment(l4_packet& in_packet,
                         std::shared_ptr<full_tcb_t> in_tcb) {
    /**
     *  SYN-RECEIVED STATE
     *  ESTABLISHED STATE
      ...
      case TCP_TIME_WAIT:
        ...
        if (segment_length == 0 && in_tcb->receiver.window > 0) {
          ...
          if (in_tcb->receiver.next <= in_tcp.seq_no &&
              in_tcp.seq_no < in_tcb->receiver.next + in_tcb->receiver.window) {
            return true;
          } else {
            return false;
          }
        ...
        if (segment_length > 0 && in_tcb->receiver.window > 0) {
          ...
          if ((in_tcb->receiver.next <= in_tcp.seq_no &&
              ...
              (in_tcb->receiver.next <= in_tcp.seq_no + segment_length - 1 &&
  ...
  void tcp_state_transition(l4_packet& in_packet,
                            std::shared_ptr<full_tcb_t> in_tcb) {
    DLOG(INFO) << "[TCP] [CHECK TCP_CLOSED] " << *in_tcb;
    if (in_tcb->state == TCP_CLOSED &&
        tcp_handle_close_state(in_packet, in_tcb)) {
      return;
    }

    DLOG(INFO) << "[TCP] [CHECK TCP_LISTEN] " << *in_tcb;
    if (in_tcb->state == TCP_LISTEN &&
        tcp_handle_listen_state(in_packet, in_tcb)) {
      return;
    }

    DLOG(INFO) << "[TCP] [CHECK TCP_SYN_SENY] " << *in_tcb;
    if (in_tcb->state == TCP_SYN_SENT &&
        tcp_handle_syn_sent(in_packet, in_tcb)) {
      return;
    }
    ...
    if (in_tcp.ACK) {
      switch (in_tcb->state) {
         ...
         *          form a reset segment, <SEQ=SEG.ACK><CTL=RST>
         */
        case TCP_SYN_RECEIVED:
        ...
        case TCP_CLOSING:
          ...
          if (in_tcp.ack_no < in_tcb->sender.unacknowledged) { /* ignore */
          }
    ...
    switch (in_tcb->state) {
         ...
         * be moved into buffers until either the buffer is full or the segment
         * is empty.  If the segment empties and carries an PUSH flag, then the
         * user is informed, when the buffer is returned, that a PUSH has been
         * received.
         *
