...
public:
    ...
    void make_response_packet(arpv4_header_t& in_header)
    {
        ...
        l2_packet out_l2_packet = l2_packet(std::nullopt, arp_header.dst_mac_addr, PROTO, std::move(out_packet));
...
struct arp_hook_t {
    ...
    void arp_filter(std::optional<l2_packet>& in_packet)
    {
        if (in_packet->_proto == arp::PROTO) {
            ...
            std::optional<ipv4_addr_t> dev_ipv4_addr = query_dev_ipv4_addr(TUNTAP_DEV);
