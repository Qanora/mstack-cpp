...
class base_hook_funcs {
 public:
  ...
  void hook_register_protocol(
      protocol_interface<CurrentPacketType>& in_protocol) {
    DLOG(INFO) << "[REGISTER] " << in_protocol.tag();
    return;
  }

  void hook_dispatch(std::optional<CurrentPacketType>& in_packet) {
    in_packet.reset();
    return;
  }
  ...
  std::optional<UpperPacketType> hook_forward_to_upper(
      std::optional<CurrentPacketType> in_packet) {
    return std::nullopt;
  }

  std::optional<CurrentPacketType> hook_forward_from_upper(
      std::optional<UpperPacketType> in_packet) {
    return std::nullopt;
  }
};
...
template <typename CurrentPacketType, typename UpperPacketType,
          typename HookFuncs =
              base_hook_funcs<CurrentPacketType, UpperPacketType>>
class layer {
 private:
  using packet_provider_current_layer_type =
      std::function<std::optional<CurrentPacketType>(void)>;
  using packet_provider_upper_layer_type =
      std::function<std::optional<UpperPacketType>(void)>;
  using packet_receiver_current_layer_type =
      std::function<void(CurrentPacketType)>;
  ...
  std::optional<packet_provider_upper_layer_type>
      _packet_provider_upper_layer_func;
 ...
 public:
  ...
  void register_dev(DEV& dev) {
    ...
    dev.register_provider_func(
        [&]() { return std::move(instance().gather_packet()); });
  ...
  void register_upper_layer(OtherLayer& layer) {
    _forward_to_upper_layer_func = [&layer](UpperPacketType packet) {
      layer.receive(std::move(packet));
    };
    _packet_provider_upper_layer_func = [&layer]() {
      return layer.gather_packet();
    };
  }
  ...
  void register_protocol(protocol_interface<CurrentPacketType>& protocol) {
    ...
    _packet_providers.push_back(
        [&protocol]() { return protocol.gather_packet(); });
    _protocols[protocol.proto()] = [&protocol](CurrentPacketType packet) {
      protocol.receive(std::move(packet));
    };

    protocol.register_forward_layer(
        [&](CurrentPacketType packet) { this->_dispatch(std::move(packet)); });
    DLOG(INFO) << "[REGISTER PROTOCOL] " << protocol.proto();
  }

  void receive(CurrentPacketType in_packet) {
    std::optional<CurrentPacketType> in_packet_ =
        std::optional<CurrentPacketType>(std::move(in_packet));
    ...
    if (this->_protocols.find(proto.value()) == this->_protocols.end()) {
      if (_forward_to_upper_layer_func) {
        DLOG(INFO) << "[FORWARD PACKET] "
                   << std::setiosflags(std::ios::uppercase) << std::hex
                   << proto.value();

        std::optional<UpperPacketType> forward_packet =
            this->hook_funcs.hook_forward_to_upper(std::move(in_packet_));

        if (forward_packet) {
          _forward_to_upper_layer_func.value()(
              std::move(forward_packet.value()));
        } else {
          DLOG(INFO) << "[FORWARD ERROR]";
        }

      } else {
        DLOG(ERROR) << "[UNKNOWN PROTOCOL] "
                    << std::setiosflags(std::ios::uppercase) << std::hex
                    << proto.value();
      }
      return;
  ...
  std::optional<CurrentPacketType> gather_packet() {
    if (this->packet_queue.empty()) {
      ...
      if (_packet_provider_upper_layer_func) {
        // LOG(INFO) << "[TEST2]";
        std::optional<UpperPacketType> in_packet =
            _packet_provider_upper_layer_func.value()();
        // LOG(INFO) << "[TEST3]";
        std::optional<CurrentPacketType> in_packet_ =
            this->hook_funcs.hook_forward_from_upper(std::move(in_packet));
        // LOG(INFO) << "[TEST4]";
        if (in_packet_) {
          // LOG(INFO) << "[TEST5]";
    ...
    std::optional<CurrentPacketType> out_packet =
        std::move(this->packet_queue.pop_front());
