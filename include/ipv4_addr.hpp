...
class ipv4_addr_t {
 ...
 public:
  ...
  void consume(uint8_t*& ptr) { _ipv4 = util::consume<uint32_t>(ptr); }

  void produce(uint8_t*& ptr) { util::produce<uint32_t>(ptr, _ipv4); }

  friend std::ostream& operator<<(std::ostream& out, ipv4_addr_t ipv4) {
    out << util::format("%u.%u.%u.%u", (ipv4._ipv4 >> 24) & 0xFF,
