...
class icmp : public protocol_interface<l3_packet> {
 public:
  ...
  virtual std::string tag() { return "ICMP"; }
  ...
  void make_icmp_reply(l3_packet in_l3_packet) {
    ...
    uint8_t* payload_pointer =
        out_icmp_header.produce(out_packet->get_pointer());
    in_l3_packet._payload->export_payload(payload_pointer,
                                          icmp_header_t::size());

    l3_packet l3_out_packet(in_l3_packet._dst_ipv4_addr,
                            in_l3_packet._src_ipv4_addr, PROTO,
                            std::move(out_packet));
...
struct icmp_hook_t {
  void icmp_filter(std::optional<l3_packet>& in_packet) {
    ...
    uint16_t checksum =
        util::checksum(pointer, in_packet->_payload->get_remaining_len(), 0);
