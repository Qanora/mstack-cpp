...
class socket_manager_t {
 ...
 public:
  ...
  void listen(int fd) {
    ...
    if (!sockets[fd].local_info) return;
    std::optional<std::shared_ptr<half_tcb_t>> tcb =
        tcb_manager.register_half_tcb(sockets[fd].local_info.value());
  ...
  //     if(fd < 0 || fd >= 200) return;
  //     if(sockets[fd].state != SOCKET_UNCONNECTED) return;
  //     full_connect_id_t full_connect(sockets[fd].remote_info.value(),
  //     sockets[fd].local_info.value());
  //     std::optional<std::shared_ptr<full_tcb_t>> tcb =
  //     tcb_manager.register_full_tcb(full_connect); sockets[fd].full_tcb =
  //     tcb; while(tcb.value()->state != TCP_ESTABLISHED){}
  // }
};
}  // namespace mstack