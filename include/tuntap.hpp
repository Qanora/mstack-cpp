...
class tuntap {
 ...
 private:
  ...
  l2_packet encode_l2_packet(uint8_t* buf, int len) {
    ...
    l2_packet l2_packet(ethernet_header.src_mac_addr,
                        ethernet_header.dst_mac_addr,
                        ethernet_header.ethernet_type, std::move(out_packet));
    ...
    while (_available) {
        ...
        if (_provider_func) {
          ...
          if (l2_packet) {
            if (l2_packet->_src_mac_addr && l2_packet->_dst_mac_addr) {
              ...
              decode_l2_packet(l2_packet.value(),
                               reinterpret_cast<uint8_t*>(_buf), len);
      ...
      if (pollevent[0].revents & POLLIN) {
        if (_receiver_func) {
          ...
          l2_packet l2_packet =
              encode_l2_packet(reinterpret_cast<uint8_t*>(_buf), n);
