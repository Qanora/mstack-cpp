...
public:
    void
    hook_register_protocol(protocol_interface<l4_packet>& in_protocol)
    {
    ...
    void
    hook_dispatch(std::optional<l4_packet>& in_packet)
    {
        return;
    }
...
{
    DLOG(INFO) << "[PROTO L4_PACKET] " << std::setiosflags(std::ios::uppercase) << std::hex << packet->_proto;
